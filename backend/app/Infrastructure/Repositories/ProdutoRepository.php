...
class ProdutoRepository extends Repository implements IProdutoRepository
{
    ...
    {
        $paginator = parent::findAll($filters, $orderBy, $limit, $page);
        
        /** @var ProdutoEntity $produto */
        foreach ($paginator['data'] as $produto) {
            $userEntity = $this->usuarioRepository->findById($produto->getUsuarioId());
            if ($userEntity) {
                $produto->setUsuario($userEntity->toArray());
            }
        }
    ...
    {
        $produto = parent::findById($id);
        if ($produto instanceof ProdutoEntity) {
            $userEntity = $this->usuarioRepository->findById($produto->getUsuarioId());
            if ($userEntity) {
                $produto->setUsuario($userEntity->toArray());
            }
     ...
     * @throws \Exception
     */
    public function findByUsuarioId(int $usuarioId, array $filters = [], array $orderBy = [], int $limit = 15, int $page = 1): array
    {
        ...
        return $this->findAll($filters, $orderBy, $limit, $page);
    }
